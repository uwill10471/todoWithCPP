...
void removeAtPos(TodoNode*& head, int index){
    ...
    if (curr == NULL) return; // Node with the given index not found
...
//to Operate Long as a user want
...
void tillExitPerformOperation(TodoNode* head){
  ...
  for(int i=0;i<5;i++){}
  switch(input){
    case 1: addNewTodo(head);
            break;
    case 2: printTodos(head);
            break;
    case 3: updateTodos(head);
            break;
    case 4: deleteTodos(head);
           break;
